%% NIE_topoplot
%
% Code adapted from UoW_topoplot, an el_topoplot function modified by Dr Frances De Blasio [email: francesd@uow.edu.au]
% Altered again by Dr Jack Fogarty [email: jacksfogarty@gmail.com] to:
%   1. Adjust marker size
%
% modified el_topoplot funciton from ERP PCA toolkit v. 2.23 (Dien, 2010)
% to suit our processing needs.  Modifications include: increasing line width 
% of contour lines (ln: 1834), and marker size of electrodes (ln: 2075). FMD Jan 2014
...
function [handle,Zi,grid,Xi,Yi] = el_topoplot(Values,loc_file,varargin)
...
EMARKERSIZE2 = 20;      % default selected channel location marker size; JSF changed from 10 to 20, 25/04/2024
EMARKER2LINEWIDTH = 1;
...
if ~strcmpi(STYLE,'blank') % if draw interpolated scalp map
 if ~strcmpi(STYLE,'grid') %  not a rectangular channel grid
  ...
  [Xi,Yi,Zi] = griddata(inty,intx,intValues,yi',xi,'v4'); % interpolate data % FMD - 'invdist' method no longer supported, changed to 'v4' instead % [Xi,Yi,Zi] = griddata(inty,intx,intValues,yi',xi,'invdist'); % interpolate data
  [Xi,Yi,ZiC] = griddata(inty,intx,intContourVals,yi',xi,'v4'); % interpolate data % FMD - 'invdist' method no longer supported, changed to 'v4' instead % [Xi,Yi,ZiC] = griddata(inty,intx,intContourVals,yi',xi,'invdist'); % interpolate data
  ...
  elseif strcmp(STYLE,'both')  % plot interpolated surface and surface contours
    ...
    warning off;
    if ~PMASKFLAG
        [cls chs] = contour(Xi,Yi,ZiC,CONTOURNUM,'k', 'LineWidth', 2); % FMD added ('LineWidth', 2) 15-01-2014
    else
...
%get(plotax,'ylim')
 ...
 if isempty(EMARKERSIZE)
   EMARKERSIZE = 25; % FMD changed 15-01-2014; was 10 
   if length(y)>=160
    EMARKERSIZE = 3;
   elseif length(y)>=128
    EMARKERSIZE = 3;
   ...
   elseif length(y)>=64
    EMARKERSIZE = 5; 
   elseif length(y)>=48
    EMARKERSIZE = 20; % FMD changed 18-04-2018 was 6
   elseif length(y)>=32 
    EMARKERSIZE = 25; 
   end
 end
